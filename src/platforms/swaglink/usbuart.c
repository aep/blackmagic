...
void do_put_fifo(char c);
void swag_init()
{
    rcc_periph_clock_enable(SWAG_SPI_CLK);
    rcc_periph_clock_enable(RCC_DMA1);

    spi_reset(SWAG_SPI);
    spi_disable(SWAG_SPI);

    //sync
    gpio_set_mode(SWAG_SPI_DIO_PORT, GPIO_MODE_OUTPUT_2_MHZ, GPIO_CNF_OUTPUT_PUSHPULL , SWAG_SPI_DIO_PIN);
    gpio_clear(SWAG_SPI_DIO_PORT, SWAG_SPI_DIO_PIN);
    delay_ms(10);

    gpio_set_mode(SWAG_SPI_CLK_PORT, GPIO_MODE_INPUT, GPIO_CNF_INPUT_FLOAT, SWAG_SPI_CLK_PIN);
    gpio_set_mode(SWAG_SPI_DIO_PORT, GPIO_MODE_INPUT, GPIO_CNF_INPUT_FLOAT, SWAG_SPI_DIO_PIN);

    spi_enable_software_slave_management(SWAG_SPI);
    spi_set_nss_low(SWAG_SPI);

    spi_set_receive_only_mode(SWAG_SPI);
    spi_set_unidirectional_mode(SWAG_SPI);
    spi_set_dff_8bit(SWAG_SPI);

    spi_set_slave_mode(SWAG_SPI);
    spi_set_clock_phase_0(SWAG_SPI); // falling edge works better, i dunno why
    spi_set_clock_polarity_0(SWAG_SPI); // low when idle
    spi_send_msb_first(SWAG_SPI);

    nvic_enable_irq(SWAG_SPI_IRQ);
    ...
    spi_enable_rx_buffer_not_empty_interrupt(SWAG_SPI);
...
void SWAG_SPI_ISR(void) {
    /* Turn on LED */
	gpio_set(LED_PORT_UART, LED_UART);

    int16_t r = spi_read(SWAG_SPI);
    if (r == 255) {
        //reset
        swag_init();
        return;
